...
if generate_ref:
    ar = h5py.File('srvo3_mesh_and_ALPS.ref1.h5', 'w')
    alps_data = np.loadtxt('srvo3_mesh_and_ALPS_maxspec.dat')
    grp = ar.create_group("A_maxspec_ALPS")
    grp.create_dataset("0", data=alps_data[:, 0])
    grp.create_dataset("1", data=alps_data[:, 1])
    grp = ar.create_group("A_maxent_lin")
    grp.create_dataset("0", data=res_lin.omega)
    grp.create_dataset("1", data=res_lin.A[1, :])
    grp = ar.create_group("A_maxent_hyp")
    grp.create_dataset("0", data=res_hyp.omega)
    grp.create_dataset("1", data=res_hyp.A[1, :])
    grp = ar.create_group("A_maxent_lor")
    grp.create_dataset("0", data=res_lor.omega)
    grp.create_dataset("1", data=res_lor.A[1, :])
    ar.close()
...
ar = h5py.File('srvo3_mesh_and_ALPS.ref.h5', 'r')
